...
//            System.out.println(errorMessage);
    ...
    @Transaction
    public ArgInfo createArgInfo(final Context ctx,final String key,String... args){
        ChaincodeStub stub= ctx.getStub();
        String argInfoState=stub.getStringState(key);
...
//            System.out.println(errorMessage);
        ...
        ArgInfo argInfo=new ArgInfo().setArgInfoID(key);
        stub.putStringState(key,JSON.toJSONString(argInfo));
        return argInfo;
    }

    @Transaction
    public ArgInfo updateArgInfo(final Context ctx,final String key,String... args){
        ChaincodeStub stub= ctx.getStub();
        String argInfoState=stub.getStringState(key);
        if(StringUtils.isBlank(argInfoState)){
            String errorMessage=String.format("ArgInfo %s does not exist",key);
