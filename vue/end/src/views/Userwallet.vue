<template>
  <div class="userindex">
    ...
    <el-row :gutter="0" class="userindex-list">
      <el-col :span="21">
        <el-table :data="admin" border stripe style="width: 100%" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="confirmId" label="ID" width="40"/>
    ...
    <el-dialog v-model="dialogVisible1" title="确认收货" width="40%">
      <el-form :model="form" label-width="80px" center>
        <el-form-item label="商品名称">
...
export default {
  ...
  methods: {
    ...
    //添加
    add() {
      request.post("/receipt/add", this.form).then(res => {
        console.log(res);
        if (res.code === 200) {
          ElMessage({
            type: 'success',
    ...
    onSubmit() {
      request.put("/confirm/update", this.form).then(res => {
        console.log(res);
        if (res.code === 200) {
          ElMessage({
            type: 'success',
    ...
    del(row) {
      this.$confirm("是否确定要删除", {
        ...
        type: "warning",
          ...
          .then(() => {
            request.delete("/confirm/delete/" + row.confirmId).then(res => {
              if (res.code === 200) {
                ElMessage({
                  type: 'success',
                  message: '删除成功',
    ...
    dele(){
      request.delete("/confirm/delete/" + this.form.confirmId).then(res => {
        if (!res.code === 200) {
          ElMessage({
            type: 'error',
            message: res.msg
    ...
    deleteBatch() {
      ...
      request.post("/confirm/deleteBatch", this.ids).then(res => {
        if (res.code === 200) {
          ElMessage({
            type: 'success',
            message: '批量删除成功',
    ...
    handleSelectionChange(val) {
      this.ids = val.map(v => v.confirmId)
    },
